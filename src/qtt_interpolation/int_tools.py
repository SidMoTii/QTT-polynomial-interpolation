...
def qtt_skcubic2d(mps, fscale,f_boundary, eps = 1e-15, order = 1, p_derivative = [0,0],boundary = 'lineare'):

    if not isinstance(f_boundary, list):
    ...
    if type(f_boundary[0]) is str:
        if f_boundary[0] == 'gaussian':
            ...
            r = 10
            f11 = 1 + (base_scaling**scale) * np.random.randn()
            bx = (1 + (base_scaling**scale)  * tntt.randn([2]*ni, [1]+[r]*(ni-1)+[1]) ) * (1-delta) + f11*delta
            by = (1 + (base_scaling**scale) * tntt.randn([2]*ni, [1]+[r]*(ni-1)+[1])) * (1-delta) + f11*delta
            corner = f11*delta
            corner2 = f11*delta2
    ...
    f_kx =  (zukron(Om1,id) @ mps) *( polsx[0] ) + ( mps *  polsx[1] ) + ( zukron(sm1,id) @ mps + mbx) * (polsx[2]) +  ( zukron(O2,id) @ mps + mbx2) * ( polsx[3])
    f_kx = f_kx.round(eps)

    By = tntt.kron( ( Om10 @ by) , pols[0] ) + tntt.kron( by , pols[1] ) +  tntt.kron( sm10 @ by + corner, pols[2]) +  tntt.kron( O20 @ by + corner2 ,  pols[3])
    By = By.round(eps)
    mby = zukron( By , deltae )
    mby2 = zukron( By , delta2e )
    
    f_kcs = (zukron(id,Om1) @ f_kx) * ( polsy[0] ) + ( f_kx * polsy[1] ) + ( zukron(id,sm1) @ f_kx + mby) * (polsy[2]) + ( zukron(id,O2) @ f_kx + mby2) * (polsy[3]) 
...
def gen_qTT_cascade_3d_p(Nscales=10, nrank=10, noise_scale=5,noctaves=5,  levels=None, seed=None,epsilon=0.1 , method = 'skc', eps=1e-10,var=1,order=1, field='stream'):
...
def skcubic3d_i(mps, fscale, f_boundary, eps = 1e-15,epsilon = 0.1, order = 1, boundary = 'periodic', p_derivative = [0,0,0]):
    ...
    if boundary == 'periodic':
        ...
        #x interpolation
        polsi = [tntt.kron( ones, zkron(tntt.TT(qtt_polynomial_cores(Mx[i], nc)) , onesf) ).round(eps) for i in range(4)]
        ...
        # y interpolation
        polsj = [tntt.kron( ones, zkron( onesf , tntt.TT( qtt_polynomial_cores(My[i], nc)) )).round(eps) for i in range(4)]
        # y interpolation
...
def skquad3d_i(mps, fscale, f_boundary, eps = 1e-15,epsilon = 0.1,boundary='lineare',p_derivative=[0,0,0]):
    ...
    polsx = [tntt.kron( ones, zkron3(qtt2pol(i,p_derivative[0]), onesf,onesf) ).round(eps) for i in range(4)]
    polsy = [tntt.kron( ones, zkron3( onesf , qtt2pol(i,p_derivative[1]), onesf )).round(eps) for i in range(4)]
    polsz = [tntt.kron( ones, zkron3( onesf , onesf , qtt2pol(i,p_derivative[2]) )).round(eps) for i in range(4)]
    ...
    if boundary == 'periodic':
        ...
        polsi = [tntt.kron( ones, zkron(qtt2pol(i,p_derivative[1]), onesf) ).round(eps) for i in range(4)]
        #interpolate boundary yz
        mlz = zkron(delta,lz)
        mlz2 = zkron(delta2,lz)
        mlz = tntt.kron(mlz,zones2)
        ...
        polsi = [tntt.kron( ones, zkron(qtt2pol(i,p_derivative[0]), onesf) ).round(eps) for i in range(4)]
        # x interpolation
        bxy= tntt.kron(bxy,zones2)
        Bxy = (zkron(Om1,id) @ bxy) *( polsi[0] ) + ( bxy *  polsi[1] ) + ( zkron(sm1,id) @ bxy + mly) * (polsi[2]) +  ( zkron(O2,id) @ bxy + mly2) * ( polsi[3])
        Bxy = Bxy.round(eps)

        pols = [qtt2pol(i,p_derivative[0]) for i in range(4)]
        ...
        polsj = [tntt.kron( ones, zkron( onesf , qtt2pol(i,p_derivative[1]) )).round(eps) for i in range(4)]
        # x interpolation
        BBxy = (zkron(id,Om1) @ Bxy) * ( polsj[0] ) + ( Bxy * polsj[1] ) + ( zkron(id,sm1) @ Bxy + mlx) * (polsj[2]) + ( zkron(id,O2) @ Bxy + mlx2) * (polsj[3]) 
        BBxy = BBxy.round(eps)
    ...
    if field == 'stream':
        ...
        for i in range(len(noisex)):
            ...
            # Interpolate to full resolution using splines interpolation
                ...
                pyAx =   skquad3d_i(am *noisex[i],Nscales, fnoise, eps=eps, epsilon=epsilon, boundary=boundary, p_derivative=[0,1,0])
                pzAx =   skquad3d_i(am *noisex[i],Nscales, fnoise, eps=eps, epsilon=epsilon, boundary=boundary, p_derivative=[0,0,1])

                pxAy =  skquad3d_i(am *noisey[i],Nscales, fnoise, eps=eps, epsilon=epsilon, boundary=boundary, p_derivative=[1,0,0])
                pzAy =  skquad3d_i(am *noisey[i],Nscales, fnoise, eps=eps, epsilon=epsilon, boundary=boundary, p_derivative=[0,0,1])

                pxAz =  skquad3d_i(am *noisez[i],Nscales, fnoise, eps=eps, epsilon=epsilon, boundary=boundary, p_derivative=[1,0,0])
                pyAz =  skquad3d_i(am *noisez[i],Nscales, fnoise, eps=eps, epsilon=epsilon, boundary=boundary, p_derivative=[0,1,0])
...
def compute_velocity_from_vector_potential_p(Ax, Ay, Az, L):
    N = Ax.shape[0]
...
@nb.njit(parallel=True)
def spectrum_binner(kmag, energy, bins):
    NB = bins.size - 1
    tot = np.zeros(NB, dtype=energy.dtype)
    # Accumulate energy into each bin
    for i in nb.prange(kmag.size):
        kv = kmag[i]
        # simple binary search for bin index
        lo, hi = 0, NB
        while hi - lo > 1:
            mid = (lo + hi) // 2
            if kv < bins[mid]:
        ...
        if 0 <= b < NB:
            tot[b] += energy[i]
    # compute centers and normalize
    centers = np.empty(NB, dtype=bins.dtype)
    E_k     = np.empty(NB, dtype=energy.dtype)
    for b in range(NB):
        centers[b] = 0.5 * (bins[b] + bins[b+1])
        E_k[b]     = tot[b] / (bins[b+1] - bins[b])
    return centers, E_k
...
@nb.njit(parallel=True)
def compute_flatness_p(u, max_sep, L):
    N = u.shape[0]
    dx = L / N
    seps  = np.empty(max_sep, dtype=np.float64)
    flats = np.empty(max_sep, dtype=np.float64)
    ...
    for dr in nb.prange(1, max_sep+1):
        ...
        # loop over every (i,j,k), wrapping i+dr mod N
        for i in range(N):
            ip = (i + dr) % N
            for j in range(N):
                for k in range(N):
