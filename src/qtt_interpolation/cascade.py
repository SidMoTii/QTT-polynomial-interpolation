...
def strandom(N, R, var = 1.0, dtype = tn.float64, device = None):
    ...
    cores = [None] * d
    for i in range(d):
        cores[i] = tn.randn([R[i],N[i][0],N[i][1],R[i+1]] if isinstance(N[i],tuple) else [R[i],N[i],R[i+1]], dtype = dtype, device = device)*np.sqrt(v**i)
...
@nb.njit(parallel=True)
def compute_flatness_box(u, max_sep, L):
    N = u.shape[0]
    dx = L / N

    # define the central box: [start, end)
    box_start = N // 4
    box_size  = N // 2
    ...
    for dr in nb.prange(1, max_sep+1):
        ...
        # if separation exceeds box size, no valid pairs
        if dr >= box_size:
            flats[dr-1] = np.nan
            continue
        ...
        S2 = 0.0
        S4 = 0.0
        # only count pairs (i,i+dr) fully inside the box
        cnt = (box_size - dr) * box_size * box_size

        for i in range(box_start, box_end - dr):
            ip = i + dr
            for j in range(box_start, box_end):
                for k in range(box_start, box_end):
                    d = u[ip, j, k] - u[i, j, k]
                    S2 += d*d
                    S4 += d*d*d*d
